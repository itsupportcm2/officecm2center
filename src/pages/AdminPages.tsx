...
export function SettingsPage(){
 ...
 const [approvers,setApprovers]=useState<Approver[]>([]);const [approverOpen,setApproverOpen]=useState(false);const [editingApprover,setEditingApprover]=useState<Approver|null>(null);const [approverName,setApproverName]=useState('');const [approverSaving,setApproverSaving]=useState(false)
 const flash=(message:string)=>{setToast(message);setTimeout(()=>setToast(''),2500)}
 const loadApprovers=async()=>{try{setApprovers(await approverService.list(true))}catch(error){flash(error instanceof Error?error.message:'โหลดรายชื่อผู้อนุมัติไม่สำเร็จ')}}
 useEffect(()=>{if(isSupabaseConfigured)settingsService.load().then(setSettings).catch(error=>flash(error instanceof Error?error.message:'โหลดการตั้งค่าไม่สำเร็จ'));void loadApprovers()},[])
 ...
 const openCreateApprover=()=>{setEditingApprover(null);setApproverName('');setApproverOpen(true)}
 const openEditApprover=(row:Approver)=>{setEditingApprover(row);setApproverName(row.name);setApproverOpen(true)}
 const saveApprover=async()=>{if(!approverName.trim()){flash('กรุณากรอกชื่อผู้อนุมัติ');return}setApproverSaving(true);try{if(editingApprover)await approverService.update(editingApprover.id,approverName,editingApprover.active);else await approverService.create(approverName);record({action:editingApprover?'UPDATE':'CREATE',entity:'settings',entityId:editingApprover?.id,title:approverName.trim(),detail:editingApprover?'แก้ไขรายชื่อผู้อนุมัติ':'เพิ่มรายชื่อผู้อนุมัติ'});await loadApprovers();setApproverOpen(false);flash(editingApprover?'แก้ไขผู้อนุมัติแล้ว':'เพิ่มผู้อนุมัติแล้ว')}catch(error){flash(error instanceof Error?error.message:'บันทึกผู้อนุมัติไม่สำเร็จ')}finally{setApproverSaving(false)}}
 const toggleApprover=async(row:Approver)=>{try{await approverService.update(row.id,row.name,!row.active);record({action:'UPDATE',entity:'settings',entityId:row.id,title:row.name,detail:row.active?'ปิดใช้งานผู้อนุมัติ':'เปิดใช้งานผู้อนุมัติ'});await loadApprovers();flash(row.active?'ปิดใช้งานผู้อนุมัติแล้ว':'เปิดใช้งานผู้อนุมัติแล้ว')}catch(error){flash(error instanceof Error?error.message:'เปลี่ยนสถานะผู้อนุมัติไม่สำเร็จ')}}
 return <div className="settings-grid">
  ...
  <section className="card approver-settings"><div className="card-head"><div><h2>รายชื่อผู้อนุมัติ</h2><p>จัดการรายชื่อที่แสดงในหน้าเบิกสินค้า</p></div><button className="btn primary compact" onClick={openCreateApprover}><UserPlus size={16}/>เพิ่มผู้อนุมัติ</button></div><div className="table-wrap"><table><thead><tr><th>ชื่อ–นามสกุล</th><th>สถานะ</th><th>จัดการ</th></tr></thead><tbody>{approvers.map(row=><tr key={row.id}><td><b>{row.name}</b></td><td><button className={`status ${row.active?'status-normal':'status-out'}`} onClick={()=>void toggleApprover(row)}>{row.active?'ใช้งาน':'ปิดใช้งาน'}</button></td><td><button className="icon-action" onClick={()=>openEditApprover(row)}><Edit3 size={16}/>แก้ไข</button></td></tr>)}</tbody></table>{!approvers.length&&<div className="empty">ยังไม่มีรายชื่อผู้อนุมัติ</div>}</div></section>
  {approverOpen&&<Modal title={editingApprover?'แก้ไขผู้อนุมัติ':'เพิ่มผู้อนุมัติ'} onClose={()=>setApproverOpen(false)}><div className="modal-body"><label>ชื่อ–นามสกุล *<input value={approverName} onChange={event=>setApproverName(event.target.value)} placeholder="กรอกชื่อผู้อนุมัติ" autoFocus/></label></div><div className="modal-actions"><button className="btn secondary" disabled={approverSaving} onClick={()=>setApproverOpen(false)}>ยกเลิก</button><button className="btn primary" disabled={approverSaving} onClick={()=>void saveApprover()}>{approverSaving?'กำลังบันทึก...':'บันทึก'}</button></div></Modal>}
  {toast&&<Toast message={toast}/>}
 </div>
}
